...
#Tool to assign a structure or a trajectory of structures into 
...
def assign(dihedrals, distances):
    ...
    for i in range(len(distances)):
        ...
        if distances[i][0] <= 1.10 and distances[i][1] <= 1.10: # unit: nm
            #can only be BABtrans
            assignment.append(7)
        elif distances[i][0] > 1.10 and distances[i][1] < 1.40:
            #can only be BBAminus
            assignment.append(6)
        else:
            #belongs to DFGin and possibly clusters 0 - 5
            mindist=10000.0
            cluster_assign = 0
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Too few arguments passed, needs the dihedrals filename and the distances filename (JSON-formatted 2D numpy arrays)")
        print("Example: python3 dunbrack_cluster.py dihedrals.json distances.json")
    else:
        obj_text = codecs.open(sys.argv[1], 'r', encoding='utf-8').read()
        myarray = json.loads(obj_text)
        dihedrals = np.array(myarray)
        obj_text = codecs.open(sys.argv[2], 'r', encoding='utf-8').read()
        myarray = json.loads(obj_text)
        distances = np.array(myarray)
        print(dihedrals)
        print(distances)
        print(assign(dihedrals, distances))
